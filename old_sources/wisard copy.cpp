...
private:
    ...
    char _code
public:
    WiSARDClassifier(int n_bits = 16, int n_tics = 256, char code = 't', int map = -1, bool debug = False)
    : _nobits(n_bits), _notics(n_tics), _code(code), _seed(map), _debu(debug) {
    }

    std::vector<int> _mk_tuple_float(float* X) {
        ...
        for (int i = 0; i < _nrams; i++) {
            for (int j = 0; j < _nobits; j++) {
                ...
                index = int(x / _notics)
                value = int((X[index] - _offsets[index]) * _notics / _ranges[index])
                intuple[i] += (x % self._notics < value) ? (1 << (_nobits - 1 - j)) : 0;
            }
        }
        return intuple;
    }

    void fit(py::array_t<float> X, const std::vector<int>& y) {
        auto buf = X.request();
        if (buf.ndim != 2) throw std::runtime_error("Input must be a 2D numpy array of floats");

        size_t num_samples = buf.shape[0];
        size_t num_features = buf.shape[1];

        // Compute ranges and offsets
        _ranges.resize(num_features);
        _offsets.resize(num_features);


        // Get pointer to NumPy data (array of array of floats)
        float* X_ptr = static_cast<float*>(buf.ptr);

        for (size_t j = 0; j < num_features; ++j) {
            float min_val = X_ptr[j], max_val = X_ptr[j];
            for (size_t i = 1; i < num_samples; ++i) {
                float val = X_ptr[i * num_features + j];
                if (val < min_val) min_val = val;
                if (val > max_val) max_val = val;
            }
            _ranges[j] = (max_val - min_val == 0) ? 1.0f : (max_val - min_val);
            _offsets[j] = min_val;
        }

        // Unique class labels and relabel `y`
        std::vector<int> unique_classes = y;
        ...
        _classes = unique_classes;
        _nclasses = _classes.size();

        // Map original labels to integer indices
        std::unordered_map<int, int> class_map;
        for (size_t i = 0; i < _classes.size(); ++i) {
            class_map[_classes[i]] = i;
        }

        // Convert `y` into zero-indexed class labels
        for (size_t i = 0; i < y.size(); ++i) {
            y[i] = class_map[y[i]];
        }
        
        // Create discriminators
        _model = WiSARD(_notics * num_features, _nobits, y, classes = _classes, map = map)

        if (_debug) {
            timing_init();
            int delta = 0; // Initialize error
        }
        for (size_t i = 0; i < num_samples; ++i) {
            if (_debug) std::cout << "Target " << y[i] << std::flush;

            float* sample_ptr = X_ptr + i * num_features; // Pointer to row
            intuple = _mk_tuple_float(sample_ptr)
            _model.train_tpl(intuple, y[i]);


            if (_debug) {
                int res = _model.test(sample_ptr, num_features);
                delta += std::abs(y[i] - res);
                timing_update(i, y[i] == res, "train ", num_samples, delta / static_cast<double>(i + 1));
            }
        }
...
PYBIND11_MODULE(wisard, m) {
    py::class_<WiSARD>(m, "WiSARD")
        ...
        .def("_mk_tuple_float", &WiSARD::_mk_tuple_float, "Make-tuple function for floats", py::arg("X"))
    ...
    py::class_<WiSARDClassifier>(m, "WiSARDClassifier")
        .def(py::init<int, int, char, int, bool>(), py::arg("n_bits") = 16, py::arg("n_tics") = 16, py::arg("code") = 't', py::arg("map") = -1, py::arg("debug") = false)
        .def("_mk_tuple_float", &WiSARDClassifier::_mk_tuple_float, "Make-tuple function for floats", py::arg("X"))
        .def("fit", &WiSARDClassifier::train, "Fit function", py::arg("X"), py::arg("y"))
    }
